...
export class ChallengeProcessor {
  ...
  static loadModels() {
    ...
    const url = "/weights/";
    promises.push(this.loadFaceDetectionModel(Utils.getConfig().FACE_DETECTION_MODEL, url));
    promises.push(this.loadLandmarkModel(Utils.getConfig().LANDMARK_MODEL, url));
    (window as any).modelPromises = promises;
  }

  private static loadFaceDetectionModel(model: string, url: string): Promise<void> {
    let promise;
    if (model === "tinyFaceDetector") {
      promise = faceapi.nets.tinyFaceDetector.load(url);
    ...
    } else if (model === "ssdMobilenetv1") {
      promise = faceapi.nets.ssdMobilenetv1.load(url);
      promise.then(function() {
        if (Utils.isProfiling()) {
          Logger.timeEnd("faceDetectionModelLoad");
        }
        Logger.info("ssdMobilenetv1 model loaded");
      });
    } else {
      throw "Invalid face detection model";
    }
    return promise;
  }

  private static loadLandmarkModel(model: string, url: string): Promise<void> {
    let promise;
    if (model === "faceLandmark68TinyNet") {
      promise = faceapi.nets.faceLandmark68TinyNet.load(url);
      promise.then(function() {
        if (Utils.isProfiling()) {
          Logger.timeEnd("faceLandmarkModelLoad");
        }
        Logger.info("faceLandmark68TinyNet model loaded");
      });
    } else if (model === "faceLandmark68Net") {
      promise = faceapi.nets.faceLandmark68Net.load(url);
      promise.then(function() {
        if (Utils.isProfiling()) {
          Logger.timeEnd("faceLandmarkModelLoad");
        }
      ...
      throw "Invalid face landmark model";
    }
    return promise;
  }
  ...
  static process(challengeProcessor: ChallengeProcessor): any {
    ...
    const options =
      Utils.getConfig().FACE_DETECTION_MODEL === "tinyFaceDetector"
        ? new faceapi.TinyFaceDetectorOptions()
        : new faceapi.SsdMobilenetv1Options();
    const useTinyModel = Utils.getConfig().LANDMARK_MODEL === "faceLandmark68TinyNet" ? true : false;
    faceapi
